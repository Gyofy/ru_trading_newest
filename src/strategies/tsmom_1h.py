"""TSMOM 1h — 24h + 6h dual momentum trend-following.

Validated: 9/9 tests passed on 14-day backtest.
- Baseline: 125 trades, WR 46.4%, Net +$1,383, PF 1.88
...
logger = logging.getLogger("strategy.tsmom_1h")


class TSMOM1h(StrategyBase):
    """24h + 6h momentum alignment on 1-hour bars."""
    ...
    async def _eval_one_coin(self, coin: str) -> Signal | None:
        df_1h = await self.data_hub.get_ohlcv(coin, "1h", limit=50)
        ...
        # RSI filter (no overbought buy / no oversold sell)
        ...
        min_move = self.config.extra.get("min_move_pct", 0.004)
        if abs(mom_24h) < min_move:
            return None

        strength = abs(mom_24h) * 100
        return Signal(
            symbol=coin,
            horizon_min=2880,
            regime=Regime.TREND,
            pred_return=abs(mom_24h),
            ...
            extra={
                "trigger": "tsmom_1h",
                "mom_24h": round(mom_24h, 6),
                "mom_6h": round(mom_6h, 6),
    ...
    def compute_barriers(
        ...
        # ATR is 1m from base class; scale to 1h
